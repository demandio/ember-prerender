...
PrerenderPool.prototype.enqueue = function(req, res, callback) {
  ...
  if (this.availableWorkers.length) {
    ...
    if (this.numWorkers < this.config.renderPoolMax) {
      // TODO: First check to see if anything is rendering
...
PrerenderPool.prototype.shrinkWorkers = function() {
  var numStopped = 0;
  while (this.availableWorkers.length > this.config.renderPoolMin) {
    var worker = this.availableWorkers.pop();
    this.stopRenderer(worker);
    numStopped++;
  }
  if (numStopped > 0) {
    this.logger.log('pool', "Shrinking pool to " + (this.numWorkers - numStopped) + " renderers");
  }
  this.shrinkTimer = null;
};
