 ...
 * Start the pool
 */
PrerenderPool.prototype.start = function(callback) {
 ...
 */
PrerenderPool.prototype.onRendererExit = function(worker, code) {
  if (code != 0) {
    this.logger.log('error', "Uncaught exception from worker, exiting now");
    process.exit(1);
  }
};
