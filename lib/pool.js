...
PrerenderPool.prototype.enqueue = function(req, res, callback) {
  ...
  if (this.availableWorkers.length) {
    ...
    if (this.numInitializingWorkers <= 1 && this.numWorkers < this.config.renderPoolMax) {
...
PrerenderPool.prototype.checkForJobs = function(worker) {
  if (this.jobs.length) {
    ...
    if (!this.shrinkTimer && this.numWorkers > this.config.renderPoolMin) {
      this.shrinkTimer = setTimeout(_.bind(this.shrinkWorkers, this), this.config.renderPoolShrinkAfter);
    }
...
/*
 * Shrink the number of available workers
 */
PrerenderPool.prototype.shrinkWorkers = function() {
  if (this.availableWorkers.length > this.config.renderPoolMin) {
...
PrerenderPool.prototype.restartRenderers = function() {
  ...
  for (var id in cluster.workers) {
    this.stopRenderer(cluster.workers[id]);
    this.startRenderer();
  }
};
