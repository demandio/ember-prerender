module.exports = {
  beforeInit: function(page, next) {
    var disableTagsRe = /<script[^<]+(?:google-analytics\.com|connect\.facebook\.net|platform\.twitter\.com|plusone\.js)[^<]+<\/script>|<link [^>]+>/g;
  ...
  beforeSend: function(page, next) {
    ...
    var stripTagsRe = /<(?:script|iframe)[^>]*>[\S\s]*?<\/(?:script|iframe)>|<!--(?! Prerender: )[\S\s]*?-->/g;
