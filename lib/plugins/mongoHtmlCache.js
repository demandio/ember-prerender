...
MongoClient.connect(mongoURI, function(err, db) {
  if (db) {
    ...
    //pages.ensureIndex({ createdOn: 1 }, { expireAfterSeconds: cacheTTL });
  }
});

module.exports = {
  beforeRender: function(req, res, page, next) {
    if (req.method !== 'GET' || !pages) {
      // Skip cache for POST/PUT requests or if no DB collection is available
      return next();
    }
    pages.findOne({ url: page.url }, function(err, result) {
  ...
  beforeSend: function(req, res, page, next) {
    ...
    pages.update({ url: object.url }, object, { upsert: true }, function (err) {
      // Ignored
    });
    next();
  }
