...
PhantomEngine.prototype.loadRoute = function(page, callback) {
  ...
  this.phantom.page.evaluate(function(pageUrl) {
    ...
    Ember.run(function() {
      router.replaceWith(pageUrl).then(null, function(rejection) {
        window.callPhantom(rejection);
      });
    });
  }, null, page.url);
};
 ...
 */
PhantomEngine.prototype.onPageReady = function(err) {
  var _this = this;
  ...
  if (this.currentCallback) {
    if (!this.loaded) {
      ...
      if (err) {
        this.logger.log('error', "Handling the URL failed with error:", err);
