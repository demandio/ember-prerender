...
JSDomEngine.prototype.init = function(html, initCallback, errorCallback) {
  ...
  try {
    ...
    this.document.addEventListener('XContentReady', _.bind(this.onPageReady, this));
...
JSDomEngine.prototype.loadRoute = function(page, callback) {
  ...
  try {
    this.window.prerenderTransitionEvent.url = page.url;
    this.window.document.dispatchEvent(this.window.prerenderTransitionEvent);
  } catch (error) {
