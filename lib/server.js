...
PrerenderServer.prototype.onRequest = function(req, res) {
  ...
  if (req.method != 'GET') {
    ...
    req.on("close", function(err) {
      if (err) {
        // TODO: Handle connection timeouts by removing them from the queue
        this.logger.log('error', "request closed with err...", err);
      }
    });
...
PrerenderServer.prototype.sendPage = function(req, res) {
  ...
  if (!res.page.statusCode || !res.page.html) {
    res.page.statusCode = 503;
    res.page.html = "503 Service Unavailable";
  }
...
/*
 * Parse the full url into the path
 */
PrerenderServer.prototype.getURL = function(reqURL) {
  ...
  // TODO: Pass query string to Ember
